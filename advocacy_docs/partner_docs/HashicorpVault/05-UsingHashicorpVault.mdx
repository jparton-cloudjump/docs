...
Before you can implement Hashicorp Vault Secrets Engine with your EDB Postgres Advanced Server 15.2 instance, you must ensure that you have the following downloaded to your system:
- Python
- [pykmip](https://pypi.org/project/PyKMIP/#files)
- edb_tde_kmip_client.py

All of the *.pem files that you created need to be on the system where your EDB Postgres Advanced Server 15.2 instance is installed.  For this guide, all of the *.pem files are in the /tmp/ directory as well as the edb_tde_kmip_client.py file.
...
In the below example, Username and Password are commented out as Hashicorp Vault does not utilize those for their connectivity:
```bash
